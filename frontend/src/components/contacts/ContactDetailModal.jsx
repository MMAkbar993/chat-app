...
export default function ContactDetailModal({
  ...
  const socials = [
    {
      name: 'Facebook', color: 'text-blue-600', url: profile?.facebook_url,
      svg: <path d="M18 2h-3a5 5 0 00-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 011-1h3z" />,
    },
    {
      name: 'Twitter', color: 'text-sky-500', url: profile?.twitter_url,
    ...
    {
      name: 'Instagram', color: 'text-pink-500', url: profile?.instagram_url,
    ...
    {
      name: 'LinkedIn', color: 'text-blue-700', url: profile?.linkedin_url, linkedinWarning: true,
    ...
    {
      name: 'YouTube', color: 'text-red-600', url: profile?.youtube_url,
      svg: <><path d="M22.54 6.42a2.78 2.78 0 00-1.95-1.96C18.88 4 12 4 12 4s-6.88 0-8.59.46a2.78 2.78 0 00-1.95 1.96A29 29 0 001 12a29 29 0 00.46 5.58A2.78 2.78 0 003.41 19.6C5.12 20 12 20 12 20s6.88 0 8.59-.46a2.78 2.78 0 001.95-1.95A29 29 0 0023 12a29 29 0 00-.46-5.58z" /><polygon points="9.75 15.02 15.5 12 9.75 8.98 9.75 15.02" /></>,
    },
    {
      name: 'Kick', color: 'text-green-500', url: profile?.kick_url,
      svg: <path d="M4 4h4v6l4-6h4l-5 7 5 9h-4l-4-7v7H4V4z" />,
    },
    {
      name: 'Twitch', color: 'text-purple-600', url: profile?.twitch_url,
      svg: <><path d="M21 2H3v16h5v4l4-4h5l4-4V2z" /><path d="M11 7v5M16 7v5" strokeLinecap="round" /></>,
    },
  ]
            ...
            <div className="space-y-3">
              {socials.filter((s) => s.url).map(({ name: sname, color, url, svg, linkedinWarning }) => (
                <div key={sname} className="flex items-center gap-3">
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${darkMode ? 'bg-gray-700' : 'bg-white'} shadow-sm ${color} shrink-0`}>

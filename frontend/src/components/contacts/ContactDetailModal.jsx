              ...
              {profile && (profile.kyc_status === 'verified' || profile.website_verified || profile.website_representation_approved || oauthSocials.some((s) => s.url)) && (
                <div className="flex flex-wrap gap-1.5">
                  {profile.kyc_status === 'verified' && (
                    <VerifiedBadge dm={darkMode} title="This user has completed identity verification before joining Pulse.">KYC Verified</VerifiedBadge>
                  )}
                  {profile.website_verified && (
                    <VerifiedBadge dm={darkMode} title="This website was verified through a meta tag or approved company representation.">Website Verified</VerifiedBadge>
                  )}
                  {profile.website_representation_approved && (
                    <VerifiedBadge dm={darkMode} title="This user has been approved to represent this company on Pulse.">Approved Rep</VerifiedBadge>
                  )}
                  {oauthSocials.some((s) => s.url) && (
                    <VerifiedBadge dm={darkMode} title="This social profile was verified through secure OAuth login.">Socials Verified</VerifiedBadge>
                  )}
                </div>
              )}

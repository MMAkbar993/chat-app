...
export default function Sidebar({ active, onNav, onEditProfile, darkMode, onDarkMode, mobileHidden }) {
  ...
  return (
    <>
    <aside className={`hidden md:flex ${collapsed ? 'w-20' : 'w-64'} shrink-0 flex-col py-5 border-r relative transition-[width] duration-200 ${darkMode ? 'bg-gray-900 border-gray-700' : 'bg-white border-gray-100'}`}>
      {/* Collapse toggle */}
      <button
        title={collapsed ? 'Expand menu' : 'Collapse menu'}
        onClick={() => setCollapsed((v) => !v)}
        className={`absolute -right-3 top-8 w-6 h-6 rounded-full flex items-center justify-center border shadow-sm transition-transform ${
          darkMode ? 'bg-gray-800 border-gray-700 text-gray-300 hover:bg-gray-700' : 'bg-white border-gray-200 text-gray-500 hover:bg-gray-50'
        }`}
      >
      ...
      {/* Logo — the full wordmark only fits once the sidebar is expanded, collapsed falls
          back to just the icon glyph */}
      <div className={`flex items-center mb-6 shrink-0 ${collapsed ? 'justify-center px-2' : 'justify-start px-5'}`}>
        <img src="/Icon.png" alt="Pulse" className={`w-8 h-8 shrink-0 ${collapsed ? '' : 'hidden'}`} />
        <img src="/full-logo.png" alt="Pulse" className={`h-9 w-auto ${collapsed ? 'hidden' : ''}`} />
      </div>

      <nav className={`flex-1 overflow-y-auto space-y-1 min-h-0 ${collapsed ? 'px-2' : 'px-3'}`}>
        {NAV.map(({ key, label, icon }) => {
          ...
          return (
            ...
            >
              ...
              <span className={`flex-1 text-left truncate ${collapsed ? 'hidden' : 'inline'}`}>{label}</span>
              {badge > 0 && (
                collapsed ? (
                  <span className={`absolute top-1.5 right-2.5 w-2.5 h-2.5 rounded-full bg-red-500 ${active === key ? '' : darkMode ? 'ring-2 ring-gray-900' : 'ring-2 ring-white'}`} />
                ) : (
                  <span className={`flex shrink-0 text-xs font-bold rounded-full min-w-5 h-5 px-1.5 items-center justify-center ${
      ...
      <div className={`pt-3 mt-1 space-y-3 border-t shrink-0 ${collapsed ? 'px-2' : 'px-3'} ${darkMode ? 'border-gray-800' : 'border-gray-100'}`}>
        ...
        >
          ...
          <span className={`flex-1 text-left ${collapsed ? 'hidden' : 'inline'}`}>Notifications</span>
          {unread > 0 && (
            collapsed ? (
              <span className={`absolute top-1.5 right-2.5 w-2.5 h-2.5 rounded-full bg-red-500 ${darkMode ? 'ring-2 ring-gray-900' : 'ring-2 ring-white'}`} />
            ) : (
              <span className="flex shrink-0 text-xs font-bold rounded-full min-w-5 h-5 px-1.5 items-center justify-center bg-red-500 text-white">
                {unread > 9 ? '9+' : unread}
              </span>
            )
          )}
        ...
        >
          ...
          <span className={`flex-1 text-left ${collapsed ? 'hidden' : 'inline'}`}>Dark Mode</span>
          <span className={`shrink-0 relative w-9 h-5 rounded-full transition-colors duration-200 ${collapsed ? 'hidden' : 'block'} ${darkMode ? 'bg-violet-600' : 'bg-gray-300'}`}>
      ...
      <button
        ...
        className={`mt-3 shrink-0 flex items-center gap-3 py-2.5 rounded-xl transition-colors ${
          collapsed ? 'mx-2 justify-center px-0' : 'mx-3 justify-start px-2.5'
      ...
      >
        ...
        <div className={`flex-1 min-w-0 text-left ${collapsed ? 'hidden' : 'block'}`}>

      ...
      <div className="flex-1 overflow-y-auto">
        {loading ? (
          ...
          <>
            ...
            {/* Social Profiles */}
            <div className={`mx-4 mb-4 p-3 rounded-xl ${card}`}>
              <p className={`text-xs font-semibold uppercase tracking-wide mb-3 ${sub}`}>Social Profiles</p>
              <div className="flex gap-4 justify-center">
                ...
                ].map(({ name: sn, color, svg }) => (
                  <button key={sn} title={sn} className={`w-10 h-10 rounded-xl flex items-center justify-center shadow-sm ${dm ? 'bg-gray-700' : 'bg-white'} ${color}`}>

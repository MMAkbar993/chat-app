...
function Chip({ icon, children, className = '' }) {
  return (
    <span className={`inline-flex items-center gap-1.5 rounded-full border border-gray-200 bg-white px-3 py-1 text-xs text-gray-700 ${className}`}>
      {icon}
      {children}
    </span>
  )
...
export default function BusinessProfilePage() {
  ...
  async function share() {
    ...
    try {
      if (navigator.share) {
        await navigator.share({ title: data.business.name, url })
        return
      }
  ...
  const header = (
    <header className="bg-white/80 backdrop-blur border-b border-gray-100">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 h-16 flex items-center justify-between">
        ...
        {authUser
          ? <Link to="/chat" className="text-sm font-semibold text-violet-600 hover:underline">Go to App</Link>
          : <Link to={`/login?next=/b/${slug}`} className="text-sm font-semibold text-violet-600 hover:underline">Sign In</Link>}
      </div>
    </header>
  )
  ...
  const { business: b, team } = data
  const websiteHref = b.website_url ? (/^https?:\/\//i.test(b.website_url) ? b.website_url : `https://${b.website_url}`) : null
      ...
      <main className="max-w-3xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          ...
          >
            ...
            <div className="relative px-5 sm:px-7 pt-20 sm:pt-24 pb-6 flex flex-col sm:flex-row sm:items-end gap-4">
              <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-2xl bg-white shadow-lg overflow-hidden shrink-0 flex items-center justify-center text-3xl font-extrabold text-gray-900">

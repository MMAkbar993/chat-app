...
export async function getProfile(req, res, next) {
  try {
    ...
    const ext = await query(
      `SELECT avatar_url, display_name, bio, gender FROM users WHERE id = $1`,
      [req.user.id]
    )
...
export async function updateProfile(req, res, next) {
  try {
    const { display_name, bio, gender, phone } = req.body
    const result = await query(
      `UPDATE users SET
         ...
         updated_at = NOW()
       WHERE id = $5
       RETURNING id, full_name, username, country, email, primary_role, phone,
                 avatar_url, display_name, bio, gender, subscription_status, kyc_status, is_active`,
      [display_name || null, bio || null, gender || null, phone || null, req.user.id]
    )
